function ssm_emfit_fast_input(sj, option)
    rng(1)
    ...
    for ms = 1:option.maxms
        ...
        for j = 0:3
            % 1) fix and slowely redcuce dynamicsnoise Sigma = diag(10^-i)
            % 2) Gamma estimated in first interation only, then fixed
            netest.S = 10^-j * eye(option.dimZ);
            
            if j == 3 % last interation variance pushed into input weigths
                option.r = option.r;
            end
            ...
            while (i==1 || LLR > tol * abs(MLL(1)) || LLR < 0) && i < MaxIter  
                ...
                if j == 0
                    netest = ssm_estimation_Classo(option, Ez, Ezz, Ezz_1, mood, inp, netest.S, []);
                else
                    netest = ssm_estimation_Classo(option, Ez, Ezz, Ezz_1, mood, inp, netest.S, netest.G);
                end
    ...
    save([option.goalpath '/fit_annealing_r' num2str(option.r) '_s' num2str(sj)], 'subject_fit');
end
