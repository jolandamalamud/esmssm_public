function netest = ssm_estimation_sad(option, z, P, P_a, X, inp, S, G)
...
%     B = xEz / Ezz_1toT; % if B is not identity
 ...
 %% latent parameter estimates
    ...
    par(:, :, i) = [A, h, C, mu0, S];
    matrixdistance = 1e10;
    while matrixdistance > tol * sum(par, 'all') && i < 6 
        i = i + 1;
        
        %       check gradients: fminopt=optimoptions('fminunc','display','off','GradObj','on','DerivativeCheck', 'on', 'FiniteDifferenceType','central')
        init = [A,C];
        est = fminunc(@(x)ssm_joint_likelihood_sad(x, option, X, inp, z, h, mu0, S, G), init, fminopt);
        A = est(:,1:option.dimZ); 
        C = est(:, option.dimZ+1:end);
        ...
        par(:, :, i) = [A, h, C, mu0, S];
        matrixdistance = sum(abs(par(:, :, i) - par(:, :, i-1)), 'all');
        disp(matrixdistance);
    end
    disp(ssm_joint_likelihood_sad([A,C], option, X, inp, z, h, mu0, S, G))
