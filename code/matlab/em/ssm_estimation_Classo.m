function netest = ssm_estimation_Classo(option, z, P, P_a, X, inp, S, G)
...
%     B = xEz / Ezz_1toT; % if B is not identity
 ...
 %% latent parameter estimates
    ...
    while matrixdistance > tol * sum(par, 'all') && i < 6 
        ...
        C = fminunc(@(x)ssm_joint_likelihood_Classo(x, option, X, inp, z, A, h, mu0, S, G), C, fminopt);
    ...
    netest.LL = ssm_joint_likelihood_Classo(C, option, X, inp, z, A, h, mu0, S, G);
